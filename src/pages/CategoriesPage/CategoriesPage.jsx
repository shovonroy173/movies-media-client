...
const CategoriesPage = () => {
  ...
  const handleClick = async (item) => {
    axios.put(
      `http://localhost:5000/api/movie/view/${item}`
    );
  ...
  const handleLike = async (item) => {
     axios.put(
      `http://localhost:5000/api/movie/like/${item}`,
  ...
  const handleDislike = async (item) => {
    axios.put(
      `http://localhost:5000/api/movie/dislike/${item}`,
      { userId: user?.currentUser._id }
    );
    // console.log("LINE AT 24", (await res).data);
  };
  const handleWishlist = async (item) => {
    axios.put(
      `http://localhost:5000/api/movie/wishlist/${item}`,
      { userId: user?.currentUser._id }
    );
    // console.log("LINE AT 24", (await res).data);
  };
